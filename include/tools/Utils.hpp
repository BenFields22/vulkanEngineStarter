...
#define FORCE_CRASH throw std::invalid_argument("Force Crash");
...
#ifdef _WIN32
    if (i)
      system("notepad AppLog.txt");
#else
    if (i)
      system("open -a TextEdit AppLog.txt");
#endif
  }

  std::string getLastCheckPoint() { return checkPoints[num]; }
};

void log(int debuging, CheckPoint *myAppLog, Logger *logger, std::string s);
extern CheckPoint *appLog;
